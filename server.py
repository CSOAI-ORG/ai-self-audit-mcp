...
mcp = FastMCP("ai-self-audit", instructions="MEOK AI Labs — AI agents audit their own compliance in real-time.")
...
@mcp.tool()
def self_audit(system_description: str, article: str = "all") -> str:
    ...
    for check, keywords in COMPLIANCE_CHECKS.items():
        ...
        results[check] = {"compliant": ok, "evidence": f"{'Found' if ok else 'Missing'} signals"}
        if ok: passed += 1
...
@mcp.tool()
def audit_conversation(text: str) -> str:
...
@mcp.tool()
def get_certificate(system_name: str, description: str) -> str:
